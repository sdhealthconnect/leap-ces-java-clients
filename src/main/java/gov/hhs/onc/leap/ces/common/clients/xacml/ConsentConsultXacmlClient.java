...
public class ConsentConsultXacmlClient {
  private static final Logger LOGGER = Logger.getLogger(ConsentConsultXacmlClient.class.getName());
  ...
  public XacmlResponse getConsentDecision(String request) throws IOException {
    ...
    if (conn.getResponseCode() != HttpURLConnection.HTTP_OK) {
      LOGGER.log(
          Level.WARNING,
          "Consent Decision XACML Failed: HTTP error code : " + conn.getResponseCode());
    ...
    return new ObjectMapper().readValue(response.toString(), XacmlResponse.class);
  }
